...
角标切「性能诊断」会显示实时数据，用于定位卡顿：

```
VidSharp · 58fps · 1.2ms · 峰14ms · 丢3 · EASU · 2560×1440 · 预备2.4s · 380MB
```

| 字段 | 含义 |
|---|---|
| `58fps` | 实际渲染帧率 |
| `1.2ms` | 单帧渲染耗时**中位数**（用中位数而非均值，偶发长帧不该主导判断） |
| `峰14ms` | 最大耗时，仅当它超过中位数 2 倍时显示 —— **这是「偶尔卡一下」的来源** |
| `丢3` | 漏掉的视频帧数，取自 `rVFC` 的 `presentedFrames` |
| `预备2.4s` | 预超分领先播放头多久 |

排查卡顿时要**区分三种情况**，只看帧率会误判：

- 渲染帧率明显低于视频帧率 → 我们跟不上（真卡顿），看 `峰` 值找尖峰
- 两者接近但都低 → 视频本身帧率低或网络卡，不是处理的问题
- `丢` 持续增长 → 有帧被跳过，通常是单帧耗时偶发超预算

角标在此模式下限流到 **1Hz** 刷新 —— fps 每帧都在变，若每帧重建纹理
（2D canvas 渲文字 + 上传 GPU）反而会制造卡顿。
...
| 模式 | 存储 | 优点 | 代价 |
|---|---|---|---|
| **无编码**（默认） | VideoFrame | 零编解码开销、无二次画质损失 | 吃显存 |
| 编码 | 压缩 chunk | 省显存（2K 每帧约 50KB） | 需硬件编码器 |

**无编码模式的缓存时长按显存预算自适应**，而非写死秒数 —— 同一个预算在
720p 能存几十秒，在 4K 只够几秒，写死必然在某一端出错：

```
预算 512MB → 2560×1440 每帧约 7MB → 可缓存约 72 帧（30fps 下 2.4 秒）
预算 512MB → 1280×720  每帧约 1.8MB → 可缓存约 284 帧（9.5 秒）
```

超预算时淘汰最旧的帧（通常已播过，丢掉无损体验）。显存预算可在 popup 里
128MB~2048MB 调节。

VideoFrame 的显存按 **每像素 2 字节**估算 —— 不是 RGBA 的 4 字节：从 canvas
构造时浏览器通常转成 NV12（YUV 4:2:0，1.5 字节/像素），取 2 留余量。

### 已知限制

- 编码模式需硬件编码器，否则不启用（会提示可改用无编码模式）
- 无编码模式受显存限制，领先量通常只有几秒
