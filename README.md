...
启用条件（**两个都必须满足**）：
- 「放大算法」选 `XLSR CNN`
- **「超分辨率」必须选 `4 倍放大`** —— 模型结构固定 4x，选 `2K`/`4K` 档时
  实际倍率不是 4，会回落 EASU

倍率不匹配时会弹提示并在控制台说明，不再静默回落。角标「详细参数」显示的是
**实际生效**的放大器（`EASU` / `RAVU` / `XLSR`），不是设置值 —— 想确认有没有
生效看这里最准。

### XLSR 适用场景

超分的价值在于「补回缺失的像素」，所以**源分辨率越低收益越大**：

| 源 | 4x 输出 | 评价 |
|---|---|---|
| 360p | 1440p | ✅ 最适合，模型先验有发挥空间 |
| 480p | 1920p | ✅ 合适 |
| 720p | 2880p | ⚠️ 收益递减 |
| 1080p | 4320p | ❌ 意义很小，且 7680×4320 极吃性能 |

**1080p 源不建议用超分**。它的问题通常不是分辨率不够，而是**码率不够** ——
该调的是去块 / 去色带 / 局部对比度，而非放大。此时「超分辨率」选 `关闭`
往往比开着更好看，因为没有插值损失，算力全用在修复上。
